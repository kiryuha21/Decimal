...
START_TEST(part1_add_normal) {
  ...
  ck_assert_int_eq(get_exponent(&result), 1);
...
START_TEST(part1_add_one_sign) {
  ...
  s21_add(value_1, value_2, &result);
  ck_assert_uint_eq(result.bits[0], test.bits[0]);
  ck_assert_int_eq(get_exponent(&result), 0);
  ck_assert_int_eq(get_sign(&result), 1);
  set_sign(&value_1, 0);
  set_sign(&value_2, 1);
  memset(&result, 0, sizeof(s21_decimal));
...
START_TEST(part1_sub_normal) {
  ...
  ck_assert_uint_eq(get_exponent(&result), 1);
...
START_TEST(part1_mult_big_power) {
  ...
  ck_assert_uint_eq(get_exponent(&result), 10);
  ck_assert_uint_eq(get_sign(&result), 0);
...
START_TEST(part1_mult_value_1_2_is_minus) {
  ...
  ck_assert_uint_eq(result.bits[0], test.bits[0]);
  ck_assert_uint_eq(get_sign(&result), 0);
  set_exponent(&value_1, 5);
  set_exponent(&value_2, 29);
  s21_mul(value_1, value_2, &result);
  ck_assert_uint_eq(result.bits[0], test.bits[0]);
}
END_TEST
...
START_TEST(part4_mod) {
  ...
  ck_assert_uint_eq(result.bits[0], test.bits[0]);
  ck_assert_uint_eq(get_sign(&result), 1);
  ck_assert_uint_eq(get_exponent(&result), 6);
  ck_assert_uint_eq(res, 0);
...
START_TEST(part2_less_pow) {
  ...
  value_1.bits[0] = 50;
  value_2.bits[0] = 30;
  set_exponent(&value_1, 8);
  set_exponent(&value_2, 8);
  ck_assert_int_eq(s21_is_less(value_1, value_2), FALSE);
...
START_TEST(part1_conv_float) {
  ...
  ck_assert_int_eq(get_sign(&temp), 0);
  ck_assert_int_eq(get_exponent(&temp), 0);
...
START_TEST(part4_negate_pow) {
  ...
  ck_assert_uint_eq(result.bits[0], test.bits[0]);
  ck_assert_int_eq(get_sign(&result), 0);
  ck_assert_int_eq(get_exponent(&result), 3);
}
END_TEST
